"""
Amazon experiment entry for ALoHA-Rec (ADLS) with multiple backbones.
...
Usage:
    python amazon_main.py --backbone all
    python amazon_main.py --backbone fcn,deepfm,dcn
    python amazon_main.py --backbone epnet --k_layers 2 --k_experts 2
"""
...
def load_amazon(data_path='./data/amazon_5_core'):
    """Load Amazon multi-domain dataset."""
    data = pd.read_csv(os.path.join(data_path, 'amazon.csv'))
    domain_num = int(data['domain_indicator'].nunique())
    cols = data.columns.values.tolist()

    dense_cols = []
    sparse_cols = [c for c in cols if c not in dense_cols + ['label', 'domain_indicator']]
    ...
    dense_feas = [DenseFeature(c) for c in dense_cols]
    sparse_feas = [SparseFeature(c, vocab_size=int(data[c].max())+1, embed_dim=16) for c in sparse_cols]
...
class DomainBalancedBatchSampler(Sampler):
    ...
    def __init__(self, domain_ids, batch_size, m_domains=4, drop_last=True):
...
def run_one_backbone(backbone, features, domain_num, train_loader, val_loader, test_loader,
    ...
    trainer = ALoHATrainer(
        model=model,
        dataset_name=f"amazon_{backbone}",
        device=device,
...
def main():
    parser = argparse.ArgumentParser(description='ALoHA-Rec Amazon Experiment')
    parser.add_argument('--data_path', type=str, default='./data/amazon_5_core')
    ...
    dense_feas, sparse_feas, X, y, domain_num = load_amazon(args.data_path)
    ...
    experiment_root = os.path.join(args.output_dir, f'aloha_rec_{timestamp}')
    ...
    with open(result_filepath, 'w') as f:
        ...
        f.write(f"Dataset: Amazon Multi-Domain CTR\n")
        f.write("="*80 + "\n\n")
