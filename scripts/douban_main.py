...
def get_douban_data_dict(data_path):
    data = pd.read_csv(os.path.join(data_path, 'douban_sample.csv'))
    ...
    dense_cols = []
    sparse_cols = ["user_id", "item_id", "living_place", "time_bin"]
    target = "rating"
...
def run_single(framework, extractor, dataset_path, epoch, lr, batch_size,
    ...
    dataset_name = "douban"
    dense_feas, sparse_feas, domain_feat, x, y, domain_num = get_douban_data_dict(dataset_path)
    ...
    trainer = ALoHATrainer(
        ...
        stage1_scheduler_step=2, stage1_scheduler_gamma=0.85,
